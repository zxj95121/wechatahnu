		...
		<div id="big">
			...
			<div id="form">
				<div id="userName" class="input">
	                <lable>账　号：</lable>
	                <input type="text" id="username" name="name" value="<?php echo $username; ?>" placeholder="请输入校园网账号" disabled="disabled" isok=1 required/>
	            </div>
			...
			$(function(){
				...
				$('#login_btn').click(function(){
					...
					if($('#username').attr('isok')==1&&$('#password').attr('isok')==1){
						...
						$('#tishi').css('opacity',1);
						$.ajax({
							url:'<?php echo httpd('/oss/cnet/valid_rebind');?>',
							type:'post',
							data:{
								username: username,
								password: password,
